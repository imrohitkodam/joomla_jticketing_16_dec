    ...
    public static function isCli()
    {
        ...
        if (is_null($isCli))
        {
            try
            {
                ...
                {
                    $isCli = version_compare(JVERSION, '1.6.0', 'ge') ? (Factory::getApplication() instanceof JException) : false;
                }
            }
            catch (Exception $e)
            {
...
function TjAkeebaStrapperLoader()
{
    ...
    if (!empty(TjAkeebaStrapper::$scriptURLs))
        foreach (TjAkeebaStrapper::$scriptURLs as $url)
        {
            ...
            {
                Factory::getDocument()->addScript($url . $tag);
            }
        }
    ...
    if (!empty(TjAkeebaStrapper::$scriptDefs))
    {
        ...
        {
            Factory::getDocument()->addScriptDeclaration($myscripts);
        }
    }
            ...
            else
            {
                if (empty($url))
                {
                    ...
                    if (!empty($altFiles))
                    {
                        ...
                        {
                            $url = FOFTemplateUtils::parsePath($altFile);
                            Factory::getDocument()->addStyleSheet($url . $tag);
                        }
                    }
                }
                else
    ...
    if (!empty(TjAkeebaStrapper::$cssDefs))
    {
        ...
        foreach (TjAkeebaStrapper::$cssDefs as $def)
        {
            ...
            {
                Factory::getDocument()->addScriptDeclaration($def . "\n");
            }
        }
        $myscripts .= '</style>' . "\n";
    }
