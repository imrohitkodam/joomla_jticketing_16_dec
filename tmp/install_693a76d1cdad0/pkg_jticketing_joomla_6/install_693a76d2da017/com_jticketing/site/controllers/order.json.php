...
class JticketingControllerOrder extends jticketingController
{
	...
	public function applyCoupon()
	{
		...
		$couponCode = $this->getInput()->get('couponCode');
		$orderId = $this->getInput()->getInt('orderId');
	...
	public function loginValidate()
	{
		...
		// Now login the user
		if (!$this->app->login(array('username' => $this->getInput()->getString('email'), 'password' => $this->getInput()->getString('password'))))
		{
	...
	{
		$book_id              = $this->getInput()->get('book_id', '', 'STRING');
	...
	{
		$result     = array();
		$amount     = $this->getInput()->get('amt', '', 'STRING');
		$totalPrice = $this->getInput()->get('totalPrice', '', 'STRING');
	...
	public function addItem()
	{
		...
		$orderId = $this->getInput()->getInt('orderId');
		$typeID  = $this->getInput()->getInt('typeId');
	...
	public function removeItem()
	{
		...
		$typeId  = $this->getInput()->get('typeId', '', 'INT');
		$orderId = $this->getInput()->get('orderId', '', 'INT');
		$ticket  = JT::tickettype($typeId);
		$order   = JT::order($orderId);
	...
	public function UpdateItem()
	{
		...
		$orderId        = $this->getInput()->getInt('orderId');
		$typeID         = $this->getInput()->getInt('typeId');
		$previousTypeId = $this->getInput()->getInt('previousTypeId');
	...
	public function getEcTrackingData()
	{
		...
		$config  = JT::config();
		$orderId = $this->getInput()->get('orderId', '', 'INT');
		$stepId  = $this->getInput()->get('stepId', '', 'INT');
		$order   = JT::order($orderId);
		$event   = JT::event()->loadByIntegration($order->event_details_id);
		$data    = new stdClass;
		$data->id = $event->id;
